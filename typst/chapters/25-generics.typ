...
#chapter("Generics", eyebrow: "Flexible, reusable code")
...
In fact, you've been using generics throughout the _Language Guide_,
...
as described in Functions\#In-Out-Parameters.
...
to write a single function that swaps two values of _any_ type.
...
uses a _placeholder_ type name (called `T`, in this case)
instead of an _actual_ type name (such as `Int`, `String`, or `Double`).
The placeholder type name doesn't say anything about what `T` must be,
but it _does_ say that both `a` and `b` must be of the same type `T`,
...
except that it can be passed two values of _any_ type,
...
the placeholder type `T` is an example of a _type parameter_.
...
is replaced with an _actual_ type whenever the function is called.
(In the `swapTwoValues(_:_:)` example above,
...
to indicate that they're a placeholder for a _type_, not a value.
...
as described in OpaqueTypes\#Opaque-Parameter-Types.
]
...
Swift enables you to define your own _generic types_.
These are custom classes, structures, and enumerations
that can work with _any_ type, in a similar way to `Array` and `Dictionary`.
...
(known as _pushing_ a new value on to the stack).
Similarly, a stack allows items to be removed only from the end of the collection
(known as _popping_ a value off the stack).
...
#image("/TSPL.docc/Assets/stackPushPop@2x.png")
...
because they need to modify (or _mutate_) the structure's `items` array.

The `IntStack` type shown above can only be used with `Int` values, however.
It would be much more useful to define a _generic_ `Stack` structure,
that can manage a stack of _any_ type of value.
...
`Stack` can be used to create a stack of _any_ valid type in Swift,
...
#image("/TSPL.docc/Assets/stackPushedFourStrings@2x.png")
...
#image("/TSPL.docc/Assets/stackPoppedOneString@2x.png")

== Extending a Generic Type

When you extend a generic type,
you don't provide a type parameter list as part of the extension's definition.
Instead, the type parameter list from the _original_ type definition
...
as discussed in Generics\#Extensions-with-a-Generic-Where-Clause below.

== Type Constraints

The `swapTwoValues(_:_:)` function and the `Stack` type can work with any type.
However, it's sometimes useful to enforce
certain _type constraints_ on the types that can be used with
...
As described in CollectionTypes\#Dictionaries,
the type of a dictionary's keys must be _hashable_.
...
see #link("https://developer.apple.com/documentation/swift/hashable\#2849490")[Conforming to the Hashable Protocol].
...
An _associated type_ gives a placeholder name
...
from Generics\#Generic-Types above,
...
as described in Protocols\#Adding-Protocol-Conformance-with-an-Extension.
...
as described in Protocols\#Declaring-Protocol-Adoption-with-an-Extension:
...
which is discussed in Generics\#Associated-Types-with-a-Generic-Where-Clause below.
...
Type constraints, as described in Generics\#Type-Constraints,
...
the items in `anotherContainer` can _also_ be checked with the `!=` operator,
...
are known as _implicit constraints_.
...
see Protocols\#Implicit-Conformance-to-a-Protocol.
...
Note that `~Copyable` doesn't _require_ the type to be noncopyable.
For example:
...
for more information,
see Declarations\#Borrowing-and-Consuming-Parameters.
