...
#chapter("Collection Types", eyebrow: "Arrays, sets, and dictionaries")
...
#image("/TSPL.docc/Assets/CollectionTypes_intro@2x.png")
...
#memo()[
Swift's array, set, and dictionary types are implemented as _generic collections_.
...
the collection that's created will be _mutable_.
This means that you can change (or _mutate_) the collection after it's created
by adding, removing, or changing items in the collection.
If you assign an array, a set, or a dictionary to a constant,
that collection is _immutable_,
...
see #link("https://developer.apple.com/documentation/swift/array\#2846730")[Bridging Between Array and NSArray].
]
...
You can also initialize an array with an _array literal_,
...
For more about the `for`-`in` loop, see ControlFlow\#For-In-Loops.

== Sets

A _set_ stores distinct values of the same type
...
see #link("https://developer.apple.com/documentation/swift/set\#2845530")[Bridging Between Set and NSSet].
]
...
A type must be _hashable_ in order to be stored in a set ---
that is, the type must provide a way to compute a _hash value_ for itself.
...
see #link("https://developer.apple.com/documentation/swift/hashable")[`Hashable`].
For information about conforming to protocols, see Protocols.
]
...
it's _only_ allowed to store `String` values.
...
#image("/TSPL.docc/Assets/setVennDiagram@2x.png")
...
Set `a` is a _superset_ of set `b`,
because `a` contains all elements in `b`.
Conversely, set `b` is a _subset_ of set `a`,
because all elements in `b` are also contained by `a`.
Set `b` and set `c` are _disjoint_ with one another,
because they share no elements in common.

#image("/TSPL.docc/Assets/setEulerDiagram@2x.png")
...
A _dictionary_ stores associations between
...
see #link("https://developer.apple.com/documentation/swift/dictionary\#2846239")[Bridging Between Dictionary and NSDictionary].
]
...
You can also initialize a dictionary with a _dictionary literal_,
...
A _key-value pair_ is a combination of a key and a value.
...
the `updateValue(_:forKey:)` method returns the _old_ value after performing an update.
