...
#chapter("Initialization", eyebrow: "Setting initial values")

_Initialization_ is the process of preparing an instance of
...
Instances of class types can also implement a _deinitializer_,
which performs any custom cleanup just before an instance of that class is deallocated.
For more information about deinitializers, see Deinitialization.

== Setting Initial Values for Stored Properties

Classes and structures _must_ set all of their stored properties
...
as shown above.
Alternatively, specify a _default property value_
...
for _every_ parameter in an initializer if you don't provide one.
...
from Initialization\#Initialization-Parameters above,
...
declare the property with an _optional_ type.
...
Swift provides a _default initializer_
...
Structure types automatically receive a _memberwise initializer_
...
This process, known as _initializer delegation_,
...
It then calls (or _delegates_) to the `init(origin:size:)` initializer,
...
_must_ be assigned an initial value during initialization.
...
as described in Initialization\#Automatic-Initializer-Inheritance below.
...
#image("/TSPL.docc/Assets/initializerDelegation01@2x.png")
...
#memo()[
These rules don't affect how users of your classes _create_ instances of each class.
...
#image("/TSPL.docc/Assets/initializerDelegation02@2x.png")
...
- term *Safety check 3*:
  ...
  before assigning a value to _any_ property
...
#image("/TSPL.docc/Assets/twoPhaseInitialization01@2x.png")
...
#image("/TSPL.docc/Assets/twoPhaseInitialization02@2x.png")
...
#memo()[
Superclass initializers _are_ inherited in certain circumstances,
but only when it's safe and appropriate to do so.
For more information, see Initialization\#Automatic-Initializer-Inheritance below.
]
...
When you write a subclass initializer that matches a superclass _designated_ initializer,
...
Conversely, if you write a subclass initializer that matches a superclass _convenience_ initializer,
that superclass convenience initializer can never be called directly by your subclass,
as per the rules described above in Initialization\#Initializer-Delegation-for-Class-Types.
...
However, superclass initializers _are_ automatically inherited if certain conditions are met.
...
- term *Rule 2*:
  ...
  _all_ of its superclass designated initializers ---
...
#image("/TSPL.docc/Assets/initializersExample01@2x.png")
...
is provided as a _designated_ initializer,
...
#image("/TSPL.docc/Assets/initializersExample02@2x.png")
...
from Initialization\#Two-Phase-Initialization above.
...
takes the same parameters as the `init(name: String)` _designated_ initializer from `Food`.
...
(as described in Initialization\#Initializer-Inheritance-and-Overriding).
...
_all_ of the designated and convenience initializers from its superclass.

The figure below shows the overall initializer chain for all three classes:

#image("/TSPL.docc/Assets/initializersExample03@2x.png")
...
isn't the same as checking for `nil` to indicate the absence of an _optional_ `String` value.
...
with a subclass _nonfailable_ initializer.
...
#image("/TSPL.docc/Assets/chessBoard@2x.png")
