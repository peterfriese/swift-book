...
#chapter("Closures", eyebrow: "Executable code blocks")

_Closures_ are self-contained blocks of functionality
...
It's explained in detail below in Closures\#Capturing-Values.
]
...
Nested functions, as introduced in Functions\#Nested-Functions,
...
_Closure expressions_ are a way to write inline closures in a brief, focused syntax.
...
if the first value should appear _before_ the second value,
and `false` otherwise.
...
The _parameters_ in closure expression syntax
...
the parameters and return type are written _inside_ the curly braces,
not outside of them.
...
For more about operator methods, see AdvancedOperators\#Operator-Methods.
...
it can be useful to write it as a _trailing closure_ instead.
...
The string-sorting closure from the Closures\#Closure-Expression-Syntax section above
...
is added to the _front_ of `output`,
...
A closure can _capture_ constants and variables
...
This means that it returns a _function_, rather than a simple value.
...
see Functions\#Function-Types-as-Return-Types.
...
It does this by capturing a _reference_ to `runningTotal` and `amount`
...
#memo()[
As an optimization,
Swift may instead capture and store a _copy_ of a value
...
Swift uses _capture lists_ to break these strong reference cycles.
For more information, see AutomaticReferenceCounting\#Strong-Reference-Cycles-for-Closures.
]
```swift
incrementByTen()
...
This is because functions and closures are _reference types_.
...
a _reference_ to the function or closure.
In the example above,
it's the choice of closure that `incrementByTen` _refers to_ that's constant,
...
A closure is said to _escape_ a function
...
see AutomaticReferenceCounting.
...
as discussed in ClassesAndStructures\#Structures-and-Enumerations-Are-Value-Types.
...
An _autoclosure_ is a closure that's automatically created
...
It's common to _call_ functions that take autoclosures,
but it's not common to _implement_ that kind of function.
For example,
...
The `@escaping` attribute is described above in Closures\#Escaping-Closures.
