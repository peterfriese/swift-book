...
#chapter("Functions", eyebrow: "Define, call, and return")
...
All of this information is rolled up into the function's _definition_,
which is prefixed with the `func` keyword.
You indicate the function's return type with the _return arrow_ `->`
...
in Functions\#Function-Argument-Labels-and-Parameter-Names
and Functions\#Default-Parameter-Values.
]
...
#memo()[
Strictly speaking, this version of the `greet(person:)` function _does_ still return a value,
...
you can use an _optional_ tuple return type to reflect the fact that
...
As you'll see in Properties\#Shorthand-Getter-Declaration,
...
Each function parameter has both an _argument label_
and a _parameter name_.
...
the argument _must_ be labeled when you call the function.

=== Default Parameter Values

You can define a _default value_ for any parameter in a function
...
The example below calculates the _arithmetic mean_
(also known as the _average_) for a list of numbers of any length:
...
define that parameter as an _in-out parameter_ instead.

You write an in-out parameter by placing the `inout` keyword
right before a parameter's type.
An in-out parameter has a value that's passed _in_ to the function,
is modified by the function,
and is passed back _out_ of the function to replace the original value.
For a detailed discussion of the behavior of in-out parameters
and associated compiler optimizations,
see Declarations\#In-Out-Parameters.
...
Every function has a specific _function type_,
...
have been examples of _global functions_, which are defined at a global scope.
You can also define functions inside the bodies of other functions,
known as _nested functions_.
