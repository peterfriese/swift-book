...
#chapter("Control Flow", eyebrow: "Branches, loops, and exits")
...
For more about arrays and dictionaries, see CollectionTypes.
...
For more about ranges, see BasicOperators\#Range-Operators.
...
However, you can use this syntax to iterate _any_ collection,
including your own classes and collection types,
as long as those types conform to the #link("https://developer.apple.com/documentation/swift/sequence")[`Sequence`] protocol.
...
This example plays a simple game of _Snakes and Ladders_
(also known as _Chutes and Ladders_):



#image("/TSPL.docc/Assets/snakesAndLadders@2x.png")
...
_before_ considering the loop's condition.
...
Here's the _Snakes and Ladders_ example again,
...
the _first_ action in the loop is to check for a ladder or a snake.
...
but this time it's not evaluated until the _end_ of the first run through the loop.
...
`square += board[square]` is always executed _immediately after_
...
To do this, you make parts of your code _conditional_.
...
as described in TheBasics\#Type-Annotations.
...
For information about working with errors, see ErrorHandling.
...
Every `switch` statement must be _exhaustive_.
...
For details, see ControlFlow\#Break-in-a-Switch-Statement.
]
...
#image("/TSPL.docc/Assets/coordinateGraphSimple@2x.png")
...
In fact, the point (0, 0) could match all _four_ of the cases in this example.
...
This behavior is known as _value binding_,
...
#image("/TSPL.docc/Assets/coordinateGraphMedium@2x.png")
...
#image("/TSPL.docc/Assets/coordinateGraphComplex@2x.png")
...
as described in Enumerations\#Associated-Values.
...
The `return` statement is described in Functions,
and the `throw` statement is described in ErrorHandling\#Propagating-Errors-Using-Throwing-Functions.
...
with a labeled `while` loop for an adapted version of the _Snakes and Ladders_ game
...
`defer` controls _when_ a piece of code is executed.
...
see ErrorHandling\#Specifying-Cleanup-Actions.
...
for the full list, see Attributes\#Declaration-Attributes.
