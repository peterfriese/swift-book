...
#chapter("Generic Parameters and Arguments", eyebrow: "Abstracting away types")
...
A generic parameter consists of a _type parameter_ followed by
an optional _constraint_. A _type parameter_ is simply the name
...
The _constraint_ specifies that a type parameter inherits
...
An _integer generic parameter_
...
The _type_ must be the `Int` type from the Swift standard library,
...
followed by a comma-separated list of one or more _requirements_.

```swift
where requirements
```

The _requirements_ in a generic `where` clause specify that a type parameter inherits from
...
see Generics\#Generic-Where-Clauses.
...
_generic-parameter_ → *`let`* _type-name_ *`:`* _type_ \
...
_same-type-requirement_ → _type-identifier_ *`==`* _signed-integer-literal_
]
...
The _generic argument list_ is a comma-separated list of type arguments.
A _type argument_ is the name of an actual concrete type that replaces
...
As mentioned in GenericParametersAndArguments\#Generic-Parameter-Clause,
...
_generic-argument_ → _type_ | _signed-integer-literal_
]
