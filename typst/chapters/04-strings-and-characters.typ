...
#chapter("Strings and Characters", eyebrow: "Fast, Unicode-compliant text")
...
see #link("https://developer.apple.com/documentation/swift/string\#2919514")[Bridging Between String and NSString].
]

== String Literals

You can include predefined `String` values within your code as _string literals_.
...
that whitespace _is_ included.

#image("/TSPL.docc/Assets/multilineStringWhitespace@2x.png")
...
- An arbitrary Unicode scalar value, written as `\u{`_n_`}`,
  where _n_ is a 1--8 digit hexadecimal number
  (Unicode is discussed in StringsAndCharacters\#Unicode below)
...
You can place a string literal within _extended delimiters_
...
You indicate whether a particular `String` can be modified (or _mutated_)
...
that `String` value is _copied_ when it's passed to a function or method,
...
Value types are described in ClassesAndStructures\#Structures-and-Enumerations-Are-Value-Types.
...
`String` values can be added together (or _concatenated_)
...
_String interpolation_ is a way to construct a new `String` value
...
_Unicode_ is an international standard for
...
Behind the scenes,
Swift's native `String` type is built from _Unicode scalar values_.
...
a single _extended grapheme cluster_.
...
However, the same letter can also be represented as a _pair_ of scalars ---
...
Each `String` value has an associated _index type_,
`String.Index`,
...
of #link("https://developer.apple.com/documentation/swift/substring")[`Substring`],
not another string.
...
#image("/TSPL.docc/Assets/stringSubstring@2x.png")

#memo()[
Both `String` and `Substring` conform to the
#link("https://developer.apple.com/documentation/swift/stringprotocol")[`StringProtocol`] protocol,
...
as described in BasicOperators\#Comparison-Operators:
...
their extended grapheme clusters are _canonically equivalent_.
...
as used in English, is _not_ equivalent to
...
the scene locations from the first two acts of Shakespeare's _Romeo and Juliet_:
...
as described in StringsAndCharacters\#String-and-Character-Equality.
]
```swift
var mansionCount = 0
var cellCount = 0
...
several Unicode-defined _encoding forms_.
Each form encodes the string in small chunks known as _code units_.
...
This process is described in StringsAndCharacters\#Working-with-Characters.
...
#image("/TSPL.docc/Assets/UTF8@2x.png")
...
#image("/TSPL.docc/Assets/UTF16@2x.png")
...
#image("/TSPL.docc/Assets/UnicodeScalar@2x.png")
