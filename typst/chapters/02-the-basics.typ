...
#chapter("The Basics", eyebrow: "Data types and syntax")
...
as described in CollectionTypes.
...
Optionals say either “there _is_ a value, which is _x_”
or “there _isn't_ a value at all”.
...
The value of a _constant_ can't be changed once it's set,
whereas a _variable_ can be set to a different value in the future.
...
see Declarations\#Constant-Declaration.
...
as described in TheBasics\#Type-Safety-and-Type-Inference.
...
surround the keyword with backticks (#raw(" ` ")) when using it as a name.
...
see Functions\#Default-Parameter-Values.
...
Swift uses _string interpolation_ to include the name of a constant or variable
...
are described in StringsAndCharacters\#String-Interpolation.
]
```swift
...
However, semicolons _are_ required
...
_Integers_ are whole numbers with no fractional component,
such as `42` and `-23`.
Integers are either _signed_ (positive, zero, or negative)
or _unsigned_ (positive or zero),
and their maximum and minimum value depends on their _size_
...
This approach is known as _fixed-point math_
...
as described in AdvancedOperators\#Overflow-Operators.
...
and matches integer type inference, as described in TheBasics\#Type-Safety-and-Type-Inference.
]

== Floating-Point Numbers

_Floating-point numbers_ have a fractional component,
...
as described in TheBasics\#Integers.
...
This kind of checking makes Swift a _type-safe_ language.
...
Swift uses _type inference_ to work out the appropriate type.
...
This is often done by assigning a _literal value_ (or _literal_)
...
Decimal floats can also have an optional _exponent_,
...
You can't pass in _any_ type here, however ---
...
_Type aliases_ define an alternative name for an existing type.
...
Boolean values are referred to as _logical_,
...
Conditional statements such as the `if` statement are covered in more detail in ControlFlow.
...
Comparisons like `i == 1` are discussed in BasicOperators.
...
_Tuples_ group multiple values into a single compound value.
The values within a tuple can be of any type
and don't have to be of the same type as each other.

In this example, `(404, "Not Found")` is a tuple that describes an _HTTP status code_.
...
For more information, see Functions\#Functions-with-Multiple-Return-Values.
...
For more information, see ClassesAndStructures.
]

== Optionals

You use _optionals_ in situations where a value may be absent.
An optional represents two possibilities:
Either there _is_ a value of a specified type,
...
it returns an _optional_ `Int`, rather than an `Int`.
...
- Propagate the `nil` value,
  by returning `nil`
  or using the `?.` operator described in OptionalChaining.
...
Optionals of _any_ type can be set to `nil`, not just object types.
]
...
the TheBasics\#Optionals section
...
_not_ to the original, optional constant or variable that you unwrapped.
...
as described in ControlFlow\#Early-Exit.
...
see BasicOperators\#Nil-Coalescing-Operator.
...
This is known as _force unwrapping_ the optional's value.
...
see TheBasics\#Assertions-and-Preconditions.
...
Sometimes it's clear from a program's structure that an optional will _always_ have a value,
...
These kinds of optionals are defined as _implicitly unwrapped optionals_.
...
as described in AutomaticReferenceCounting\#Unowned-References-and-Implicitly-Unwrapped-Optional-Properties.
...
described above in TheBasics\#Type-Safety-and-Type-Inference,
Swift also protects code against working with invalid memory.
This protection is known as _memory safety_
...
discussed in TheBasics\#Error-Handling
and TheBasics\#Assertions-and-Preconditions below.
However, in some cases,
the _only_ safe way to handle an error is to stop execution.
...
You use _error handling_ to respond to error conditions
...
That function's caller can then _catch_ the error and respond appropriately.
...
_Assertions_ and _preconditions_
...
within the code.
Unlike the error conditions discussed in TheBasics\#Error-Handling above,
...
#link("https://developer.apple.com/documentation/swift/1541112-assert")[`assert(_:_:file:line:)`] function
...
you use the
#link("https://developer.apple.com/documentation/swift/1539616-assertionfailure")[`assertionFailure(_:file:line:)`] function
...
but must _definitely_ be true for your code to continue execution.
...
#link("https://developer.apple.com/documentation/swift/1540960-precondition")[`precondition(_:_:file:line:)`] function.
You pass this function an expression that evaluates to `true` or `false`
and a message to display if the result of the condition is `false`.
For example:
...
#link("https://developer.apple.com/documentation/swift/1539374-preconditionfailure")[`preconditionFailure(_:file:line:)`] function
